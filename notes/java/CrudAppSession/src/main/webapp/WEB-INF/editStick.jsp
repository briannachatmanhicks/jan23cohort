...
<body>
	<header>
		...
		<main>
		<h1>Edit Sticker</h1>
		<form:form action="/sticker/${s.id}/update" method="post" modelAttribute="editStickForm">
			<input type="hidden" name="_method" value="put">
			<section>
				<form:label path="name">Sticker Name</form:label>
				<form:errors path="name"/>
				<form:input path="name" value="${s.name}"/>
			</section>
			<section>
				<form:label path="URL">Image URL</form:label>
				<form:errors path="URL"/>
				<form:input path="URL" value="${s.URL}"/>
			</section>
			
			<section>
				<form:label path="theCat">Category Name</form:label>
				<span>Current Value = ${s.theCat.name}</span>
